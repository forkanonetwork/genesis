...
NODE_NAME="Forkano Node"                  # Change your node's name prefix here, keep at or below 19 characters!
...
RETRIES=3                                 # How many attempts to connect to running Forkano node before erroring out and quitting (0 for continuous retries)
...
usage() {
  cat <<-EOF
		...
		Guild LiveView - An alternative forkano-node LiveView
...
node_version=$(grep "forkano-node" <<< "${version}" | cut -d ' ' -f2)
...
while true; do
  ...
  if [[ ${check_peers} = "true" ]]; then
    ...
    printf "${VL}    the forkano-node port. Should give ~100%% success rate." && closeRow
